...
Use **Expand all folders** or **Collapse all folders** for the complete tree. Folders with nested folders have a compact tree control immediately before their normal disclosure arrow; it expands or collapses that complete branch, while clicking the folder name still changes only that one level. Leaf folders omit the redundant tree control. Every folder also has a tri-state **Select all** control: it selects or deselects every file in that folder and all of its subfolders, including descendants hidden inside collapsed branches.
