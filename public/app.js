...
function renderAdminManager() {
  ...
  const branch = (node, depth = 0) => {
    ...
    const treeControl = nestedFolders.length ? `<button class="admin-folder-branch" data-admin-folder-branch="${esc(node.path)}" aria-label="${treeAction} ${esc(node.path)} and all nested folders" title="${treeAction} this folder tree"><span>${expandTree ? "+" : "−"}</span></button>` : `<span class="admin-folder-branch-spacer" aria-hidden="true"></span>`;
    return `<section class="admin-folder-node"><div class="admin-folder-row visibility-${visibilityStatus}" style="--depth:${depth}">${treeControl}<button class="admin-folder-main" data-admin-folder="${esc(node.path)}" aria-expanded="${!collapsed}"><span class="admin-folder-chevron">${hasChildren ? (collapsed ? "▶" : "▼") : "·"}</span><span class="admin-folder-icon">▱</span><strong>${esc(node.name)}</strong><small class="admin-folder-summary"><span></span>${visibleCount}/${descendantPaths.length} visible</small></button><button class="admin-folder-select ${selectionStatus === "all" ? "is-all" : selectionStatus === "some" ? "is-partial" : ""}" data-admin-folder-selection="${esc(node.path)}" aria-pressed="${selectionStatus === "some" ? "mixed" : selectionStatus === "all"}" aria-label="${selectAction} all ${descendantPaths.length} files in ${esc(node.path)}" title="${selectAction} this folder and all subfolders" ${descendantPaths.length ? "" : "disabled"}><span>${selectionStatus === "all" ? "✓" : selectionStatus === "some" ? "−" : ""}</span><b>${selectAction} all</b></button></div><div class="admin-children ${collapsed ? "is-collapsed" : ""}">${node.folders.map((child) => branch(child, depth + 1)).join("")}${node.files.map((file) => fileRow(file, depth + 1)).join("")}</div></section>`;
  };
